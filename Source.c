...
void render(int cells[], int size)
{
	...
	for(c = 0; c < size; c++)
	{
//		cells[c] == 0 ? eraseCell(c % ys, c / ys) : drawCell(c % ys, c / ys);
...
int main(int argc, char *argv[])
{
	...
	mvprintw(rows/2 - 1,cols/2 - 3, "CONWAY");
	attroff(A_BOLD);
	mvprintw(rows/2 + 1,cols/2 - 16, "Powered by Destructive Reasoning");
